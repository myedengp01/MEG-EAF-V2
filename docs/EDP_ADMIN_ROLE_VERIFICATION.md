...
Existing HR Letters release blockers in PR #1 (legal entity/salary mapping, template reproducibility and production verification) remain. Administrator navigation and the draft preview race are fixed with local regression coverage; full-app browser acceptance remains open. The isolated workflow test does not resolve those separate issues.
