...
After review, validate the migration and authenticated RPC in staging, then deploy database before UI. Verify the Access Log and protected super-admin on that environment. Irene must select the actual person before any real role assignment.

Existing HR Letters release blockers in PR #1 (navigation, legal entity/salary mapping, preview race, template reproducibility and production verification) remain. The isolated workflow test does not resolve those separate issues.
