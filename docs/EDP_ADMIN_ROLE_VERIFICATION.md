...
Existing HR Letters release blockers in PR #1 (legal entity/salary mapping, full staging verification and production release approval) remain. Administrator navigation and the draft preview race are fixed with local regression coverage. All 15 inactive templates now have an exact snapshot and tested restore generator; migration packaging and full staging restore remain pending (see HR_LETTERS_TEMPLATE_RESTORE.md). Full-app browser acceptance remains open. The isolated workflow test does not resolve those separate issues.
