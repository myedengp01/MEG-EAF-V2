...
V2.version='MEG-EAF V2 HR v2026.08.31-02';
V2.jd=null;
...
function findEntityId(code){
  ...
  if(!x && code==='meh'){
    var m=(COMPANY_ALIASES.meg||[]).map(norm);
    x=(V2.jd.entities||[]).find(function(e){return m.indexOf(norm(e.name))>=0;});
  }
  return x?x.id:null;
}
...
function ensureSummaryCompanyColumn(){
  ...
  var th=document.createElement('th'); th.setAttribute('data-v2-company','1'); th.style.cssText='border:1px solid #ccc;padding:5px 8px;text-align:left;'; th.textContent='Legal Employer'; tr.insertBefore(th,tr.firstChild);
}
window.loadSummary=function(){
  ...
  apiFetch('/rest/v1/eaf_applications?select=id,status,payload,submitted_at&order=submitted_at.desc').then(function(rows){
    ...
    tbody.innerHTML=rows.map(function(r){var p=r.payload||{},code=p.companyCode||'meg',c=companies()[code]||{};var cells=[
      c.name||code,summaryField(p,'nameEnglish'),summaryField(p,'nameChinese'),summaryIcOrPassport(p),calcAge(summaryField(p,'dob')),summaryField(p,'email'),summaryField(p,'contact'),summaryGender(p),summaryRace(p),summaryAddress(p),summaryField(p,'finalStart'),summarySalary(p),summaryField(p,'approvedJobTitle'),summaryWorkingHours(p),summaryField(p,'finalDept'),summaryField(p,'decision'),r.status
...
window.onEafCompanyChange=function(){if(typeof oldCompanyChange==='function')oldCompanyChange();applyProfileDefaults(true);renderHrApplicantPosts();if(window.isAdmin)renderAppointmentLetter();};
...
window.populateJobTitleDropdown=function(){
  ...
  var current=sel.value,code=companyCode(),eid=findEntityId(code),rows=[];
  if(V2.jd&&eid){rows=(V2.jd.positions||[]).filter(function(p){return p.entity_id===eid && (!p.status||p.status==='approved');});}
  if(rows.length){
    var groups={};rows.forEach(function(p){var d=deptName(p.department_id)||'Positions';(groups[d]||(groups[d]=[])).push(p);});
    var html='<option value="">-- Select approved job title --</option>';
    Object.keys(groups).sort().forEach(function(g){html+='<optgroup label="'+e2(g)+'">'+groups[g].map(function(p){return '<option value="jd:'+e2(p.id)+'">'+e2(p.level+' — '+p.job_title)+(p.reference?' ['+e2(p.reference)+']':'')+'</option>';}).join('')+'</optgroup>';});
    html+='<option value="__OTHER__">Other (type manually)</option>';sel.innerHTML=html;
    if(current&&Array.prototype.some.call(sel.options,function(o){return o.value===current;}))sel.value=current;
  }else{
    if(typeof oldPopulateJob==='function')oldPopulateJob();
  }
};

var oldResolve=window.resolvePosition;
window.resolvePosition=function(){
  var sel=byId('approvedJobTitle'),val=sel?sel.value:'';
  if(val&&val.indexOf('jd:')===0&&V2.jd){
    var id=val.slice(3),p=(V2.jd.positions||[]).find(function(x){return String(x.id)===id;});
    if(p){var resp=(V2.jd.responsibilities||[]).filter(function(r){return String(r.position_id)===String(p.id);}).map(function(r){return {k:r.title||'Responsibility',v:r.description||''};});
      return {code:null,title:p.job_title||'',lvl:p.level||'',purpose:p.purpose||'',respItems:resp,deptName:deptName(p.department_id),jdId:p.id,jdReference:p.reference||'',reportsTo:p.reports_to||'',employmentType:p.employment_type||'',location:p.location||''};}
  }
  return oldResolve();
};
...
window.onJobTitleChange=function(){
  var sel=byId('approvedJobTitle'),val=sel?sel.value:'';
  if(val&&val.indexOf('jd:')===0){
    if(byId('jobTitleOtherWrap'))byId('jobTitleOtherWrap').style.display='none';
    var p=resolvePosition(),ls=byId('jobTitleLevel');
    if(ls){ls.innerHTML='<option value="'+e2(p.lvl)+'|'+e2(p.title)+'">'+e2(p.lvl+' — '+p.title)+'</option>';ls.selectedIndex=0;}
    if(byId('finalDept')&&p.deptName){byId('finalDept').value=p.deptName;byId('finalDept').dataset.auto='1';}
    if(byId('reportingTitle')&&!byId('reportingTitle').value&&p.reportsTo)byId('reportingTitle').value=p.reportsTo;
    if(byId('v2EmploymentType')&&p.employmentType&&!byId('v2EmploymentType').dataset.userSet)byId('v2EmploymentType').value=p.employmentType;
    if(byId('v2JdReference'))byId('v2JdReference').value=p.jdReference||'';
    window.onLevelChange(); return;
  }
  oldJobTitleChange();
  var rp=resolvePosition();if(byId('v2JdReference'))byId('v2JdReference').value=rp.jdReference||'';
};

function renderHrApplicantPosts(){
  var host=byId('postGroup');if(!host)return;
  var eid=findEntityId(companyCode()),rows=[];
  if(V2.jd&&eid)rows=(V2.jd.positions||[]).filter(function(p){return p.entity_id===eid&&(!p.status||p.status==='approved');});
  if(!rows.length)return; // retain V1 fallback until JD master is available
  var selected=(V2.pendingRestore&&V2.pendingRestore.cb_post)||Array.prototype.map.call(document.querySelectorAll('input[name="post"]:checked'),function(x){return x.value;});
  var groups={};rows.forEach(function(p){var g=deptName(p.department_id)||'Available Positions';(groups[g]||(groups[g]=[])).push(p);});
  var html='';Object.keys(groups).sort().forEach(function(g){html+='<div style="width:100%;font-weight:700;color:#1565c0;margin:8px 0 4px">'+e2(g)+'</div>';groups[g].forEach(function(p){html+='<label><input type="checkbox" name="post" value="'+e2(p.job_title)+'" data-jd-id="'+e2(p.id)+'"> '+e2(p.job_title+(p.level?' ('+p.level+')':''))+'</label>';});});
  html+='<label style="width:100%"><input type="checkbox" name="post" value="Other / Position not listed"> Other: <input type="text" id="postOtherDynamicHr" style="width:210px;border:none;border-bottom:1px solid #555;margin-left:4px"></label>';host.innerHTML=html;
  Array.prototype.forEach.call(host.querySelectorAll('input[name="post"]'),function(cb){if(selected.indexOf(cb.value)>=0)cb.checked=true;});
}

// V2 form persistence --------------------------------------------------------
var oldCollect=window.collectFormData;
window.collectFormData=function(){var d=oldCollect();d.v2Schema='2.0';[
...
].forEach(function(id){var el=byId(id);if(el)d[id]=el.value;});var rp=resolvePosition();d.v2JdId=rp.jdId||'';d.v2JdReference=rp.jdReference||d.v2JdReference||'';d.v2Handbook=profile().handbook;d.v2OffDay=profile().offDay;d.v2RestDay=profile().restDay;return d;};
var oldRestore=window.restoreFormData;
window.restoreFormData=function(d){V2.pendingRestore=d||{};oldRestore(d||{});setTimeout(applyPendingRestore,0);setTimeout(applyPendingRestore,400);};
function applyPendingRestore(){var d=V2.pendingRestore||{};[
  'v2EmploymentType','v2FixedTermEnd','v2ProbationMonths','v2ProbationNotice','v2ProbationNoticeUnit','v2OtCategory','v2FixedAllowance','v2OtherAllowance','v2JdReference'
].forEach(function(id){if(byId(id)&&d[id]!==undefined)byId(id).value=d[id];});
  if(d.approvedJobTitle&&byId('approvedJobTitle')){var sel=byId('approvedJobTitle');if(Array.prototype.some.call(sel.options,function(o){return o.value===d.approvedJobTitle;})){sel.value=d.approvedJobTitle;onJobTitleChange();}}
...
window.buildLocePage1HTML=function(d){var c=d.company||companyObj(),p=d.entityProfile||profile();var fixed=d.employmentType==='Fixed-Term Full-Time'&&d.fixedTermEnd?'<tr><td>Employment Term</td><td>'+e2(d.employmentType)+' until '+e2(d.fixedTermEnd)+'</td></tr>':'<tr><td>Employment Type</td><td>'+e2(d.employmentType)+'</td></tr>';return ''
  ...
  +'<tr><td>Total Fixed Monthly Remuneration</td><td>'+d.totalFixedFmt+'</td></tr><tr><td>Working Hours</td><td>From '+d.timeFrom+' to '+d.timeTo+', subject to lawful event/project scheduling where applicable</td></tr><tr><td>Normal Off Day / Rest Day</td><td>'+e2(p.offDay)+' / '+e2(p.restDay)+'</td></tr><tr><td>Place of Work</td><td>'+d.workLocation+'</td></tr><tr><td>Reporting Personnel</td><td>'+d.reportingName+'</td></tr></table>';};
