...
V2.version='MEG-EAF V2 HR v2026.08.31-04';
V2.jd=null;
...
function renderHrApplicantPosts(){
  ...
  if(V2.jd&&eid){
    positions=(V2.jd.positions||[]).filter(function(p){return p.entity_id===eid&&(!p.status||p.status==='approved');});
    var deptIds={};positions.forEach(function(p){deptIds[String(p.department_id)]=true;});
    families=(V2.jd.departments||[]).filter(function(d){return d.entity_id===eid&&deptIds[String(d.id)]&&d.is_active!==false;}).slice().sort(function(a,b){return String(a.name||'').localeCompare(String(b.name||''));});
  }
  if(!families.length)return;

  var pending=V2.pendingRestore||{};
  var selected=(pending.cb_post||[]).slice();
  var selectedIds=(pending.selectedPostFamilyIds||[]).map(String);
  var legacyDeptIds={};
  // Build 03 compatibility: map an old exact applicant JD title back to its general family.
  selected.forEach(function(v){
    var p=positions.find(function(x){return x.job_title===v;});
    if(p)legacyDeptIds[String(p.department_id)]=true;
  });

  var html='<div style="width:100%;font-weight:700;color:#1565c0;margin:8px 0 5px">Applicant Intended Post (General)</div>';
  families.forEach(function(d){
    html+='<label><input type="checkbox" name="post" value="'+e2(d.name)+'" data-post-family-id="'+e2(d.id)+'"> '+e2(d.name)+'</label>';
  });
  ...
  Array.prototype.forEach.call(host.querySelectorAll('input[name="post"][data-post-family-id]'),function(cb){
    if(selected.indexOf(cb.value)>=0||selectedIds.indexOf(String(cb.dataset.postFamilyId))>=0||legacyDeptIds[String(cb.dataset.postFamilyId)])cb.checked=true;
  });
  var combined=pending.postOtherManual||pending.postOtherSelection||pending.postOtherDynamic||pending.postOther||pending.postOtherOps||'';
...
window.collectFormData=function(){var d=oldCollect();d.v2Schema='2.0';d.v2RecruitmentSchema='general-post-family-v1';[
  ...
  var rp=resolvePosition();d.v2JdId=rp.jdId||'';d.v2JdReference=rp.jdReference||d.v2JdReference||'';d.v2Handbook=profile().handbook;d.v2OffDay=profile().offDay;d.v2RestDay=profile().restDay;return d;};
