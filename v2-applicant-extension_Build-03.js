...
(function(){
  ...
  V2.version = 'MEG-EAF V2 Applicant v2026.08.31-03';
  ...
  function loadPublicJdData(){
    ...
    return Promise.all([
      ...
      sbPublic('/rest/v1/jd_positions?select=id,entity_id,department_id,job_title,level,reference,status,is_open_for_recruitment&status=eq.approved&is_open_for_recruitment=eq.true&order=entity_id,department_id,level,job_title')
    ]).then(function(parts){
  ...
  function renderPostsFromRows(code, rows, deptMap){
    ...
    if(!rows.length){
      html+='<div style="width:100%;padding:8px 10px;background:#fff8e1;border:1px solid #ffe082;border-radius:4px;color:#6d4c41;font-size:11px">No positions are currently open for recruitment for this company. You may still specify another position below for HR consideration.</div>';
    }
  ...
  function loadCompanyPosts(code){
    ...
    // Build 03: the applicant sees only JDs deliberately published as Open for Recruitment.
    // A successful empty result means there are no open vacancies for the selected legal employer.
    // If the publishing RPC is unavailable, fail closed rather than exposing the full JD catalogue.
    return loadPublishedVacancies().then(function(publicRows){
      ...
      renderPostsFromRows(code,converted,deptMap);
    }).catch(function(err){
      console.warn('MEG-EAF V2 vacancy publishing unavailable:',err);
      renderPostsFromRows(code,[],{});
    });
  }
